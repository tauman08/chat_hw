...
def main():
    ...
    try:
        ...
        LOG.critical((f'неверно указана командная строка')) if LOGGING_ON else getpass
        sys.exit(2)
    except ValueError:
        LOG.critical((f'Запуск сервера с неверным портом {srv_port}. ДОпустимы номера портов с 1024 по 65535')) if LOGGING_ON else getpass
    ...
    try:
        ...
        LOG.critical((f'Не указан адрес сервера')) if LOGGING_ON else getpass
        sys.exit(2)
    ...
    try:
        while True:
            ...
            try:
                ...
                LOG.debug(f'Получено сообщение: {message}') if LOGGING_ON else getpass
                response = process_message(message)
                LOG.debug(f'Сформирован ответ клиенту: {response}') if LOGGING_ON else getpass
                send_message(client, response)
                LOG.debug(f'Закрытие клиентского {client_address} соединения') if LOGGING_ON else getpass
                client.close()
            except (ValueError, json.JSONDecodeError):
                LOG.critical(f'От клиента {client_address} пришли некорректные данные') if LOGGING_ON else getpass
                client.close()
    finally:
        listen_socket.close()
